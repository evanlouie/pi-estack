    ...
    while i < len(lines):
        ...
        if raw.startswith((" ", "\t")):
            add_issue(
                ...
                f"Unexpected indented top-level line: {stripped}",
            )
            i += 1
            continue
        ...
        if not match:
            add_issue(
                ...
                "warning",
                "frontmatter-parse",
                f"Could not parse frontmatter line: {raw}",
            )
            i += 1
            continue
...
def extract_skill_file(path: Path, issues: list[Issue]) -> tuple[Path, Path] | None:
    ...
    if not skill_file.exists():
        add_issue(
            issues, "error", "missing-skill-md", f"SKILL.md not found at {skill_file}"
        )
        return None
...
def split_frontmatter(
    text: str, issues: list[Issue], skill_file: Path
) -> tuple[str, str]:
...
def validate_frontmatter(
    data: dict[str, Any], skill_dir: Path, issues: list[Issue]
) -> None:
    ...
    if not name:
        add_issue(
            issues, "error", "missing-name", "Frontmatter field `name` is required"
        )
    else:
        if len(name) > 64:
            add_issue(
        ...
        if skill_dir.name != name:
            add_issue(
                ...
                f"`name` is `{name}` but parent directory is `{skill_dir.name}`",
            )
    ...
    if not description:
        ...
        if not re.search(
            r"\b(use|when|asks|requested|needs|handle|create|write|analyze|process)\b",
            description,
            re.I,
        ):
            add_issue(
                issues,
                "warning",
                "description-may-not-trigger",
                "Description may not clearly state when to use the skill",
            )
    ...
    if compatibility and len(compatibility) > 500:
        add_issue(
            ...
            f"`compatibility` is {len(compatibility)} characters; max is 500",
        )
    ...
    if metadata and not isinstance(metadata, dict):
        add_issue(
            ...
            "warning",
            "metadata-not-map",
            "`metadata` should be a key-value mapping",
        )
...
def validate_body(body: str, skill_dir: Path, issues: list[Issue]) -> None:
    if not body.strip():
        add_issue(
            ...
            "warning",
            "empty-body",
            "SKILL.md body is empty; add task instructions",
        )
        return
    ...
    if line_count > 500:
        add_issue(
            ...
            "warning",
            "body-too-long",
            f"SKILL.md body has {line_count} lines; consider moving details to references/",
        )
    if word_count > 3500:
        add_issue(
            issues,
            "warning",
            "body-may-exceed-token-guidance",
            f"SKILL.md body has about {word_count} words; consider progressive disclosure",
        )
    ...
    for rel in sorted(set(PATH_RE.findall(body_for_refs))):
        ...
        if not target.exists():
            add_issue(
                ...
                f"Referenced file `{rel_clean}` does not exist",
                rel_clean,
            )
    ...
    for script in sorted(p for p in scripts_dir.rglob("*") if p.is_file()):
        ...
        if script.suffix == ".py":
            ...
            if python_calls_input(text):
                add_issue(
                    ...
                    f"Python script `{rel}` calls input(); avoid interactive prompts",
                    rel,
                )
            if "argparse" not in text and "--help" not in text:
                add_issue(
                    issues,
                    "warning",
                    "script-help-not-obvious",
                    f"Script `{rel}` may not document a --help interface",
                    rel,
                )
        ...
        if script.suffix != ".py" and ("read -p" in text or "prompt(" in text):
            add_issue(
                issues,
                "warning",
                "script-may-be-interactive",
                f"Script `{rel}` may prompt interactively",
                rel,
            )
        ...
        try:
            ...
            if not isinstance(data, list):
                add_issue(
                    issues,
                    "error",
                    "eval-queries-not-list",
                    "evals/eval_queries.json must be a JSON list",
                )
            else:
            ...
            add_issue(
                issues,
                "error",
                "eval-queries-invalid-json",
                f"evals/eval_queries.json is invalid JSON: {exc}",
            )
    ...
    if evals.exists():
        try:
            ...
            if (
                ...
                or "skill_name" not in data
                or "evals" not in data
            ):
                add_issue(
                    issues,
                    "error",
                    "invalid-evals-json",
                    "evals/evals.json must contain skill_name and evals",
                )
        except json.JSONDecodeError as exc:
            add_issue(
                issues,
                "error",
                "evals-invalid-json",
                f"evals/evals.json is invalid JSON: {exc}",
            )
...
def print_human(
    skill_dir: Path | None, issues: list[Issue], metadata: dict[str, Any]
) -> None:
    if skill_dir:
